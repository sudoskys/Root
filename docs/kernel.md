...
!!! warning
    本部分内容对小白可能太难了，建议有一定知识储备后再来。

!!! warning
    在操作本部分时，由于每个系统的环境都有差别，很可能会出现无法预料的奇怪报错，对此我们无法解决。除非你严格按照本教程操作之后还有问题，否则请不要提问。
...
#### 看前须知
---
首先，你需要确认你的手机内核是开源的。如果不开源，那就不用继续看了。
...
!!! warning
    要注意内核是和安卓版本对应的，不要找错了。

!!! warning
    有些设备有多个版本的内核可用(例如红米Note7可用的版本有 `4.4`,`4.9`,`4.14`,`4.19`)，这些内核版本和 ROM 是对应的，刷入其他版本会导致无法开机，所以你应该记住你当前的内核版本。
...
#### 准备环境
---
编译 Linux 内核肯定是需要 Linux 的，目前无非就是这几种方法：

- 实机安装一个 Linux 发行版(推荐 Ubuntu)
- 使用 Windows 的 Linux 子系统v2(WSL2)安装一个Linux发行版(同样推荐 Ubuntu)
- 手机上 Chroot 安装一个 Linux 发行版(还是推荐 Ubuntu)

除非迫不得已，否则我不建议你用手机编译来折磨自己。

如果电脑配置不大行，建议实机安装。

对于如何安装以上的系统，本教程不多赘述，请直接谷歌。
...
```
其中 `--depth=1` 表示浅克隆，即不克隆提交记录，因为 Linux 历史悠久，提交记录非常多(比如一加8T的内核有83万多个提交)如果将他们克隆下来，将会非常慢，最后源码目录也会非常大。
